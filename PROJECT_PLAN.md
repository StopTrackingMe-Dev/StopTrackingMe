...
- 不上传屏幕截图；
- 不上传目标 App 节点树；
- 可选使用统计必须先取得用户明确同意，拒绝不得影响任何功能；
- 使用统计仅汇报分享操作次数和随机安装实例码，不汇报链接、分享目标或内容；
- 统计后端不持有原始安装实例码或设备标识符，仅以实例码的服务端 HMAC 作为统计键；
- 调试快照仅在开发构建中启用；
- 发布构建关闭详细节点日志。
...
产生的剪贴板 URL。链接参数净化在设备本地完成，链接不会被持久化保存，链接
内容不会发送给开发者或使用统计后端。展开已知短链时，应用会直接请求短链
服务商，不经过开发者服务器，用户可关闭此功能。
生成分享网页卡片预览时，应用会直接访问净化链接所属网站及规则明确允许的
封面图片 CDN，以读取公开标题、摘要和封面；请求不经过开发者服务器，预览
内容仅在本次分享会话的内存中保留。
用户可自行选择是否启用隐私保护的使用统计。启用后，应用会生成与设备和账号
无关的随机安装实例码，并仅汇报通过本应用发起分享操作的次数。统计请求不包含
链接、标题、剪贴板内容、来源或目标应用、联系人、截图或无障碍界面数据；后端
不保存原始安装实例码，仅以服务端密钥计算的不可逆 HMAC 作为统计键；为网络
重试去重而保存的事件收据保留约 90 天并每日清理。拒绝或关闭统计不影响任何
功能，关闭时应用会请求删除对应统计记录。
QQ 好友与 QQ 空间分享由腾讯 QQ 互联 SDK 提供。SDK 只会在用户同意后启用，
可能获取设备型号并检查 QQ/TIM 安装情况；分享时会将用户确认的标题、摘要、
净化链接和封面交给 QQ。用户可在应用首页撤回 QQ SDK 授权。
